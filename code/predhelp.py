...
def gdp_clean (csv_data_url):
    ...
    #impute values using KNNImputer and matching the new df to the old df
    imputer = KNNImputer()
    dfimputed = imputer.fit_transform(gdp13_wide_drop)
    gdp13_wide_imp = pd.DataFrame(dfimputed)
    #for the countries with more than one NaN we used MICE Imputer
    ...
    #putting both imputed and correct values together into one DF (Repeating each column thrice because we have to scale quarterly data to monthly data)
    ...
    #adding numbers 1 to 12 into the df and then converting it to a datetime date
...
def show_na(dfs):
    subset_df = dfs.loc[:, dfs.isnull().any()]
    print(subset_df.columns.values.tolist(), "has one or multiple NaN values")
...
def unemployment_clean (csv_data_url):
    ...
    for i in unemployment_rate_wide.columns[unemployment_rate_wide.isnull().any(axis=0)]:
